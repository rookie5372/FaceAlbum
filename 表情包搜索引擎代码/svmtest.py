...
def testsvm(svm, root):
	...
	# re = svm.get_support_vector_count()
	# re2 = svm.get_var_count()
	return result

def singletest(svm, imagepath):
	test_img = []
	...
	hog = Hog_descriptor(img, cell_size=size, bin_size=16)
	vector, img = hog.extract()
	test_img.append(vector[0])
	test_img = np.float32(np.array(test_img))
	result = svm.predict_all(test_img)
	return result

def svmmainsearch(image_name):
	pklfile = open('classhash.pkl','rb')
	testhash = pickle.load(pklfile)
	# pprint.pprint(testhash)

	svmmodel = getsvm('svm_face.dat')
	result = np.int0(singletest(svmmodel, image_name)).ravel()
	return testhash[result[0]]

# for searching using .pkl and .dat
if __name__ == "__main__":
	...
	svmmodel = getsvm('svm_face.dat')
	print 'load success'
	result = np.int0(singletest(svmmodel, 'test.jpg')).ravel()

	# result = np.int0(testsvm(svmmodel, 'test').ravel())
